            ...
            <div class="card-heading pt-2">
               ...
               <img src="{{ asset('avatars/graduate.png') }}" class="ml-2" alt="avatar" width="50" height="50">
                ...
                @if(Auth::id() == $d->user->id)
                   ...
                   @if(!$d->hasBestAnswer())
                      <span class="float-right mr-2"><a href="{{ route('discussion.edit',['id'=>$d->id]) }}" class="btn btn-success btn-sm"> Edit </a></span>
                   @endif

                @endif
                
                @if($d->hasBestAnswer())
                <span class="float-right text-white bg-danger py-1 px-2 mr-2">Closed</span>
                @else
                <span class="float-right text-white bg-success py-1 px-2 mr-2">Open</span>
                @endif

                <span class="float-right mr-2"><a href=" {{ route('discussion',['slug'=>$d->slug]) }} " class="btn btn-info btn-sm ">View</a></span>
               <hr>
